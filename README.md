...
- **发现页（默认落地）**：第一排「继续阅读」（当前在读，只显示一行），下面「随手翻翻」随机一批，可「换一批」重排。解决大书库每次打开都一样、老书被埋没的问题。
- **书架墙**：网格展示书库，显示阅读进度（封面「注水」效果）、上次阅读时间、状态；支持搜索 / 筛选（全部·在读·未读·读完）/ **按分类筛选（= 顶层文件夹，纯前端从路径推导）** / 排序。**虚拟滚动**，上万本也流畅、内存平稳；首屏优先加载书籍、统计延后。
- **系列（多卷套装）**：程序启发式检测套装候选（文件名同构 / 卷号前缀 / 文件夹名前缀 / 套装字样，含「每卷一个子文件夹」的嵌套结构），在设置页**一键确认**后生效——绝不自动归组，归错可随时取消。确认后的文件夹（含子层级）在书架收成一张卡（册数徽标 + 读完数 + 整体进度），点开按自然序列出各卷（第2卷不会排在第10卷后）；搜索时仍可搜到单卷。
- **封面与元数据**：PDF 渲染首页；EPUB / CBZ 解压取内嵌封面；MOBI / AZW3 解析 PDB+EXTH 抽取内嵌封面与书名/作者（文件名乱码的电子书据此纠正标题）。全部在独立工作进程完成，不阻塞界面。
- **扫描索引**：递归扫描库目录，识别 PDF / EPUB / MOBI / AZW3 / DjVu / CBZ / **TXT**（小于 5KB 的 txt 视为说明文件跳过）。多段式——先秒级入库（书架立即可浏览），再后台抽取电子书元数据、补算 PDF 总页数。基于文件大小+修改时间增量更新。SumatraPDF 原生支持 txt，进度同步与自动结束照常。
- **阅读会话**：点击书籍 → 用外部阅读器打开并开始计时，卡片播放音乐波纹动画；也可随时手动结束。**自动结束**（默认阈值 10 分钟）信号：① 整机键鼠空闲超时——人离开了，对所有书/阅读器生效（时长计到最后一次输入）；② 阅读器进程被关闭——child exit + 按进程名轮询双探测，覆盖 ReuseInstance（立即结束）；③ 系统休眠（立即结束）；④ 翻页空闲——**仅当本次会话已收到 ≥3 个实时翻页信号才参与判定**（SumatraPDF 主要在关文档/退出时才写盘，「没收到信号」≠「没在读」，不能凭它误停）。被空闲自动结束的书在窗口期内再翻页会**自动恢复**；冷信号（如 Sumatra 关闭时的批量回写）只同步进度、不凭空开会话。**试读**：未读的书一次读不满试读时间（默认 10 分钟）就结束的话，自动保持「未读」不转「在读」，提示里可一键反悔；时长仍计入统计。
- **进度自动同步**：监听 SumatraPDF 的 `SumatraPDF-settings.txt`，解析 `FileStates` 的 `PageNo`，自动回写每本书的当前页与进度。同步**只改页码/进度，不改阅读状态**（状态由会话、试读与手动设置管理；达到 99.5% 自动判「读完」且有粘性）。注意 Sumatra 并非实时写盘（主要在关文档/退出时批量回写），所以阅读中进度条不会实时上涨，关书后才追平。
- **书籍详情**：查看完整信息、阅读历史（每次时长/页码区间）、手动设置当前页。
- **统计面板**：日/周/月阅读时长柱状图、近一年阅读热力图、阅读时长排行、累计时长、连续阅读天数、在读/读完数。
- **设置**：库目录（多目录）、默认阅读器 + **按格式指定阅读器**（如 AZW3→Calibre）、空闲结束阈值、试读时间、SumatraPDF 设置文件、扫描间隔、数据目录。
