...
- **阅读会话**：点击书籍 → 用外部阅读器打开并开始计时，卡片播放音乐波纹动画；也可随时手动结束。**自动结束**（默认阈值 5 分钟）有四个信号，任一命中即结束、动画停止：① 有同步信号的书超时没翻页（时长计到最后一次翻页，再次翻页**自动恢复**）；② 整机键鼠空闲超时——人离开了，对所有书/阅读器生效（时长计到最后一次输入）；③ 拉起的阅读器进程被关闭（立即结束）；④ 系统休眠（立即结束）。**试读**：未读的书一次读不满试读时间（默认 10 分钟）就结束的话，自动保持「未读」不转「在读」，提示里可一键反悔；时长仍计入统计。
